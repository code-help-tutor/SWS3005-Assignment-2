...
VBOSphere2::VBOSphere2(float rad, GLuint sl, GLuint st) :
...
void VBOSphere2::generateVerts(float * verts, float * norms, float * tex,
                               unsigned int * el)
{
    ...
    for( GLuint i = 0; i < slices; i++ ) {
        ...
        for( GLuint j = 0; j < stacks; j++ ) {
            if( j == 0 ) {
                el[idx] = stackStart;
                el[idx+1] = nextStackStart + 1;
                el[idx+2] = stackStart + 1;
                idx += 3;
            } else if( j == stacks - 1) {
                el[idx] = stackStart + j;
                el[idx+1] = nextStackStart + j;
                el[idx+2] = stackStart + j + 1;
                idx += 3;
            } else {
                el[idx] = stackStart + j;
                el[idx+1] = nextStackStart + j + 1;
                el[idx+2] = stackStart + j + 1;
                el[idx+3] = nextStackStart + j;
                el[idx+4] = nextStackStart + j + 1;
                el[idx+5] = stackStart + j;
                idx += 6;
            }
        }
    }
...
int VBOSphere2::getVertexArrayHandle() {
